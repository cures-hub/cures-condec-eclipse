	...
	private void createLinks(Node node, int currentDepth, int maxDepth, Set<Node> visitedNodes) {
		...
		if (currentDepth < maxDepth && !visitedNodes.contains(node)) {
			...
			if (node instanceof JiraIssue) {
				...
				Set<GitCommitImpl> commits = gitClient.getCommitsForIssueKey(ji.getIssueKey());
				...
				for (IssueLink il : issue.getIssueLinks()) {
					JiraIssue ji2 = JiraIssue.getOrCreate(IssueKey.getOrCreate(il.getTargetIssueKey()), jiraClient);
					if (ji2 != null) {
